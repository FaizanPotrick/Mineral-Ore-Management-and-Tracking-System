  ...
  actions: {
    ...
    async dashboard_fetch() {
      const {
        ...
        `/api/dashboard/${$cookies.get("type_of_user")}${$cookies.get("type_of_user") === "officer"
    ...
    async mines_fetch() {
      const { data } = await axios.get(
        `/api/mines/${$cookies.get("type_of_user")}${$cookies.get("type_of_user") === "officer"
          ? "/" + $cookies.get("type_of_region")
          : ""
        }`
      );
          ...
          if (user_type === $cookies.get("type_of_user")) {
            if ($cookies.get("type_of_user") === "officer") {
              for (let i = 0; i < button.type_of_region.length; i++) {
                if (button.type_of_region[i] === $cookies.get("type_of_region")) {
                  return true;
                }
              }
              return false;
