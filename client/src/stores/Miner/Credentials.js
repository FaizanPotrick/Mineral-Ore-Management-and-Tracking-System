...
export const useCredentialStore = defineStore({
  ...
  actions: {
    ...
    async register() {
      ...
      open_alert_text("", "");
      this.isLoading = true;
      const res = await fetch("http://localhost:8000/miner_registeration", {
        method: "POST",
        headers: {
    ...
    async logIn() {
      const { email_address, password } = this.login;
      open_alert_text("", "");
      this.isLoading = true;
      const res = await fetch("http://localhost:8000/miner_login", {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
