...
const onDecode = async (result) => {
  ...
  const { status, data } = await axios.get(
    `/api/verify/transaction?transaction_id=${resultParse.transaction_id}&transaction_hash=${resultParse.transaction_hash}`
  );
  if (status === 200) {
...
const register_fn = async () => {
  ...
  try {
    await axios.get(
      `/api/registration/${
        $cookies.get("type_of_user") === "organization"
          ? "organization"
          : "checkpoint"
      }/transaction?transaction_id=${transaction.value._id}`
    );
    ...
    >
      ...
      <table class="my-5 min-w-full text-gray-900">
        <tbody>
          ...
          <tr>
            <td class="mr-6 py-1 font-bold">Organization ID:</td>
            <td class="py-1">
