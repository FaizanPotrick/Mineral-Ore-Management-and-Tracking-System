...
const searchList = () => {
  const data = filteredOfficers.value.filter((item) => {
    if (search.value === "") {
      ...
      item.type_of_region.toLowerCase().includes(search.value.toLowerCase())
    ) {
      return true;
    ...
    >
      <div class="flex justify-between items-center w-full">
        ...
        <div class="hidden sm:block">
          <input
            type="search"
            class="w-10vw px-4 py-2 border border-gray-300 rounded-lg"
            placeholder="Search"
            v-model="search"
        ...
        <table class="w-full">
          ...
          <tbody class="whitespace-nowrap">
            <tr
              ...
              @click="
                router.push(
                  `/dashboard/officers/${officer.type_of_region}/${officer._id}`
                )
              "
            >
