<script setup>
import useDashboardStore from "@/stores/Dashboard/HomeStore";
import { ref } from "vue";
const search = ref("");
useDashboardStore().organisations_fetch()

function filterList() {
    return useDashboardStore().mine_list.filter(mine => {
            ...
            <div class="border rounded-xl overflow-hidden drop-shadow-md">
                <table>
                    ...
                    <tbody class="font-normal text-gray-600 whitespace-nowrap">
                        <tr :key="organisation._id" v-for="organisation in useDashboardStore().organisations"
                            class="text-center">
                            <td class="px-6 py-4">
                                {{ organisation._id }}
                            </td>
