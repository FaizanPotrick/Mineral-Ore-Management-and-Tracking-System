    ...
    <div class="flex justify-center items-center min-h-[86vh] bg-yellow-50">
        <div class="max-w-4xl p-10 bg-white border border-gray-400/20 shadow-md rounded-2xl m-5 sm:10 text-gray-800">
            <div class="mb-4">
                ...
                <div class="text-gray-500 text-sm">Fill your ore transaction details.</div>
                <span :class="{
                ...
                }" class="text-center text-sm" v-if="useAlertStore().alert_text.message &&
...
">{{ useAlertStore().alert_text.message }}</span>
            ...
            <form class="space-y-5 drop-shadow-md" @submit.prevent="register_fn()">
                <div class="grid gap-6 mb-6 grid-cols-1">
                    <div class="space-y-2">
                        <label class="text-sm font-medium text-gray-700">Amount of Ore</label>
                        <input type="text"
                            class="w-full text-base px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-yellow-600"
                            placeholder="Amount of Ore" maxlength="20"
                            @change="useValidationStore().validation(amount_of_ore)" v-model="amount_of_ore.value"
                            required />
                        <span class="text-center text-sm text-red-500" v-if="!amount_of_ore.valid">{{
                                amount_of_ore.message
                        }}</span>
                    </div>
                </div>
                <div class="grid gap-6 sm:grid-cols-2">
                    <div class="space-y-2">
                        <label class="text-sm font-medium text-gray-700">Type of Ore</label>
                        <input type="text"
                            class="w-full text-base px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-yellow-600"
                            placeholder="Type of Ore" maxlength="50"
                            @change="useValidationStore().validation(type_of_ore)" v-model="type_of_ore.value"
                            required />
                        <span class="text-center text-sm text-red-500" v-if="!type_of_ore.valid">{{
                                type_of_ore.message
                        }}</span>
                    </div>
                    <div class="space-y-2">
                        ...
                        <input type="text"
                            class="w-full text-base px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-yellow-600"
                            placeholder="Grade" minlength="1" maxlength="3"
                            @change="useValidationStore().validation(grade)" v-model="grade.value" required />
                        <span class="text-center text-sm text-red-500" v-if="!grade.valid">{{
                                grade.message
                        }}</span>
                    </div>
                </div>
                <div class="grid gap-6 grid-cols-1">
                    <div class="space-y-2">
                        ...
                        <input type="text"
                            class="w-full text-base px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-yellow-600"
                            placeholder="Price" maxlength="150" @change="useValidationStore().validation(price)"
                            v-model="price.value" required />
                        <span class="text-center text-sm text-red-500" v-if="!price.valid">{{ price.message
                        }}</span>
                    </div>
                </div>
