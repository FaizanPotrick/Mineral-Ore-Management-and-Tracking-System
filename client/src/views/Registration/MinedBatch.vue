...
const store_document = (event) => {
  mined_batch.value.mine_lap_report = event.target.files[0];
};
...
const register_fn = async () => {
  ...
  formData.append("fe_percentage", mined_batch.value.fe_percentage);
  formData.append(
    "grade",
    mined_batch.value.fe_percentage >= 65
      ? "high"
      : mined_batch.value.fe_percentage >= 62 &&
        mined_batch.value.fe_percentage < 65
      ? "medium"
      : "low"
  );
  ...
  formData.append("mine_lap_report", mined_batch.value.mine_lap_report);
  await axios({
    method: "post",
