...
onMounted(async () => {
  const response = await fetch("/api/registration/region")
  const { states, districts } = await response.json();
  useMinerStore().states_list = states
  useMinerStore().districts_list = districts
})
const {
  manager_name,
      ...
      <form class="space-y-5 drop-shadow-md" @submit.prevent="register_fn()">
        <div class="grid gap-6 grid-cols-1">
          <div class="space-y-2">
            ...
            <input type="text"
              ...
              placeholder="Organization ID" v-model="useMinerStore().organization_id" required />
          </div>
          <div class="space-y-2">
        ...
        <div class="grid gap-6 md:grid-cols-3 sm:grid-cols-2">
          <div class="space-y-2">
            ...
            <select
              class="w-full text-base px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-yellow-600"
              @change="district_selector" v-model="useMinerStore().mine_state"
              :disabled="(useMinerStore().states_list).length === 0" required>
              <option disabled selected value="">Choose a State</option>
              <option v-for="state in useMinerStore().states_list" :value="state.state">
                {{ state.state }}
              </option>
            </select>
          </div>
          <div class="space-y-2">
            ...
            <select
              class="w-full text-base px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-yellow-600"
              @change="coordinates_selector" v-model="useMinerStore().mine_district"
              :disabled="(useMinerStore().district_list).length === 0" required>
              <option disabled selected value="">Choose a District</option>
              <option v-for="district in useMinerStore().district_list" :value="district.district">
                {{ district.district }}
              </option>
            </select>
          </div>
          <div class="space-y-2">
            ...
            <input type="text" placeholder="Pin Code"
              class="w-full text-base px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-yellow-600"
              @change="useValidationStore().validation(mine_pin_code)" :disabled="useMinerStore().mine_district === ''"
              v-model="mine_pin_code.value" required />
            <span class="text-center text-sm text-red-500" v-if="!mine_pin_code.valid">{{
                mine_pin_code.message
            }}</span>
          </div>
          <div class="space-y-2">
        ...
        <GMapMap :center="useMinerStore().center" :zoom="useMinerStore().zoom" map-type-id="terrain"
          style="width: 100%; height: 50vh" @click="marker_selector">
          <GMapMarker v-if="useMinerStore().coordinates.lat !== 0 && useMinerStore().coordinates.lng !== 0"
