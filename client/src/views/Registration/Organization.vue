...
const register_fn = async () => {
  loading.value = true;
  try {
    await axios.post("/api/registration/organization", organization.value);
    organization.value = {
      organization_name: "",
      address: "",
      name: "",
...
<template>
  <div class="flex justify-center items-center min-h-[86vh]">
    <div
      class="max-w-2xl w-full p-10 bg-white border border-gray-400/20 shadow-md rounded-2xl m-5 sm:10 text-gray-800"
    >
      <div class="mb-4">
        <div class="font-semibold text-2xl text-yellow-700">
          Organization Registration
        </div>
        <div class="text-gray-500 text-sm">Register an Organization.</div>
      </div>
