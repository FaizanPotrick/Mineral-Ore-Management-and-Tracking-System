...
const register_fn = async () => {
  loading.value = true;
  try {
    await axios.post("/api/registration/mine/transaction", transaction.value);
    transaction.value = {
      organization_id: "",
      type_of_ore: "",
      grade: "",
...
onMounted(async () => {
  const { data } = await axios.get("/api/transaction/organizations");
    ...
    >
      ...
      <form class="space-y-5 drop-shadow-md" @submit.prevent="register_fn()">
        <input
          ...
          v-model="transaction.organization_id"
          required
          list="org_id_list"
        />
        ...
        <div class="grid gap-6 sm:grid-cols-2">
          <select
            class="w-full px-3 py-1.5 border rounded-lg focus:outline-none focus:border-yellow-600/40"
            v-model="transaction.type_of_ore"
            required
          >
          ...
          <select
            class="w-full px-3 py-1.5 border rounded-lg focus:outline-none focus:border-yellow-600/40"
            v-model="transaction.grade"
            required
          >
            <option disabled value="" selected>Select the Grade</option>
          ...
          <input
            class="w-full px-3 py-1.5 border rounded-lg focus:outline-none focus:border-yellow-600/40"
            v-model="transaction.vehicle_no"
            type="text"
        ...
        <button
          ...
          :disabled="loading"
        >
          <span v-if="!loading" class="h-6"> Create Transaction </span>
          <span v-else>
            <svg
